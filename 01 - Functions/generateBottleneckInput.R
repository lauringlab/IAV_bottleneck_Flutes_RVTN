generateBottleneckInput <- function(final_pairs, df, save = FALSE) {
  ...
  for (p in 1:nrow(final_pairs)) {
    ...
    pair_snv <- full_join(donor_snv, recip_snv) %>%
      ...
      mutate(
        donor_freq = case_when(
          ...
          both == FALSE & only_donor == TRUE & same_cons == TRUE ~ 1 - donor_freq,
          both == FALSE & only_donor == TRUE & same_cons == FALSE & dAlt_rCons == TRUE ~ donor_freq,
          
          both == FALSE & only_donor == FALSE & same_cons == TRUE ~ 1,
        ...
        recip_freq = case_when(
          ...
          both == FALSE & only_donor == TRUE & same_cons == TRUE ~ 1,
          both == FALSE & only_donor == TRUE & same_cons == FALSE & dAlt_rCons == TRUE ~ 1,
          
          both == FALSE & only_donor == FALSE & same_cons == TRUE ~ 1 - recip_freq,
    ...
    # 5. Save data to folders --------------------------------------------------
    if (save == TRUE) {
      pair_export <- pair_snv %>%
        select(donor_freq, recip_freq)
      
      file_name <- paste("03 - Input/processedData/",
