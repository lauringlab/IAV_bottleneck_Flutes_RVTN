...
source("./scripts/00-consensusSequences.R")
# rm(list=setdiff(ls(), c("all_sequences", "output_dataset")))

raw_data <- read.csv("./updatedData/input/SNV_with_meta_data.csv")
...
donor_mutations <- donor_ids %>% 
    ...
    dplyr::rename(donor_ref_allele = ref_allele, 
           donor_alt_allele = alt_allele,
           donor_freq = avg_freq,
           donor_id = full_id)

recipient_mutations <- recipient_ids %>% 
    dplyr::rename(full_id = recipient) %>% 
    inner_join(df, multiple = "all") %>% 
    select(pair_id, full_id, year, household, genome_segment, segment_position,
           ref_allele, alt_allele, avg_freq) %>% 
    dplyr::rename(recipient_ref_allele = ref_allele, 
...
final_sequences_long <- final_sequences_wide %>% 
    ...
    na.omit() %>% 
    dplyr::rename(segment_position = position) %>% 
...
donor_consensus <- df2 %>% 
    ...
    dplyr::rename(donor_cons_allele = base,
           donor = full_id)
...
recipient_consensus <- df2 %>% 
    ...
    dplyr::rename(full_id = recipient) %>% 
    left_join(final_sequences_long) %>% 
    dplyr::rename(recipient_cons_allele = base,
           recipient = full_id)
...
# Create data frame in final form for bottleneck calculations                ---
df6 <- df5 %>% 
    filter(only_recipient != TRUE) %>%
    select(pair_id, year, household, donor_freq2, recipient_freq2)
...
for(i in pairs) {
    file_name <- paste("./processedData/",
                       paste(paste("pair", i, sep = "_"), ".txt", sep = ""),
                       sep = "")
    df_pair <- df6 %>%
        filter(pair_id == i) %>%
        select(donor_freq2, recipient_freq2)
    write.table(df_pair,
                file = file_name,
                sep = "\t",
